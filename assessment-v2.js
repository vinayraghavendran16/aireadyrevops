...
const QUESTIONS = [
  ...
  {
    dim: 'data',
    text: "If you asked three of your reps to define MQL right now, would you get three matching answers? Your AI inherits whichever definition it's fed.",
  ...
  {
    dim: 'ai',
    text: "Of the AI seats you're actually paying for, what percentage of reps use them as a core part of their daily workflow?",
...
function computeResults() {
  ...
  let band;
  if      (index >= 80) band = { label: 'AI-Ready',     cls: 'ready',     desc: 'Your foundation supports the AI you bought. The spend is positioned to return — focus on scaling and advanced use cases. This is the band where AI investment actually compounds.' };
  else if (index >= 65) band = { label: 'AI-Capable',   cls: 'capable',   desc: 'Most of your foundation holds. One or two gaps are skimming the return off your AI spend. Targeted fixes turn money you are already spending into measurable ROI.' };
  else if (index >= 50) band = { label: 'AI-Risky',     cls: 'risky',     desc: 'Your AI spend is at real risk of returning unreliable output. The foundation cannot fully support what is running on it — structural fixes are needed before you add more AI.' };
  else if (index >= 35) band = { label: 'AI-Fragile',   cls: 'fragile',   desc: 'The AI you are paying for is likely producing unreliable output today. The return is leaking faster than you can measure it. Intervene before any new spend.' };
  else                  band = { label: 'AI-Premature', cls: 'premature', desc: 'There is no foundation yet for AI spend to return against. New tools will compound waste, not ROI. Fix the foundation before spending another dollar on AI.' };
